...
void showCamera(float &camX, float &camY)
{
	...
	WAIT(2000);
	log_to_file("finished keep camera...");
	// get far and near clip
	float Far = CAM::GET_CAM_FAR_CLIP(cameraHandle);
	float Near = CAM::GET_CAM_NEAR_CLIP(cameraHandle);
	std::ofstream outfile;
	outfile.open("cameraClip.txt");
	outfile << "Far" << std::endl;
	outfile << Far << std::endl;
	outfile << "Near" << std::endl;
	outfile << Near << std::endl;
	outfile.close();
}
