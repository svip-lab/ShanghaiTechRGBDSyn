...
void scriptMain()
{
	...
	bool partGet = InitPartNo();  // parts
	set_status_text("GCC-CL start");
	...
	{	
		...
		if (scriptStatus == scriptStart) {
			while (1) {
				...
				WAIT(1000);
				if (scriptStatus == scriptStop) {  
					break;  // after create a scene with serveral environment
				}
			}
		...
		// sleep mode
			...
			if (++sleepTime == 20000) {
				// log_to_file("sleep 20000 game ms...");
