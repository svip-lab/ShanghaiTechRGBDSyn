...
bool createPeds(int &n, float &sx, float &dx, float &sy, float &dy) {
	float x, y, avgz = readZheight() + 0.5;  // Zheight.log
...
void createImgFold(char *foldName, char* pedInfoFile, char* rawPathTemp, WCHAR* imgPathTemp, char* depthPathTemp, char* matrixPathTemp, char* envDepthTemp) {
	...
	// matrix file name
	strcpy(matrixPathTemp, foldName);
	strcat(matrixPathTemp, "matrix.txt");

	// environment depth file
	strcpy(envDepthTemp, foldName);
	strcat(envDepthTemp, "depth_env.raw");
	...
	// depth file name
	strcpy(depthPathTemp, foldName);
	strcat(depthPathTemp, "depth_%d.raw");
...
// capture background only
void screenCap(int id, WCHAR* imgPathTemp, char* rawPathTemp, char* depthPathTemp, char* envDepthTemp) {
	...
	char depthPath[fileLength];
	sprintf(depthPath, rawPathTemp, id);
	char envDepthPath[fileLength];
	sprintf(envDepthPath, envDepthTemp);

	int t = 0;
	do {
		// capture image only
		catchCurveAndScreen(imgPath, rawPath, depthPath, NULL, envDepthPath, true, true);  // catchCurveAndScreen
		//catchCurveAndScreen(imgPath, rawPath, depthPath, NULL, true, true);
...
void screenStencilCap(int id, WCHAR* imgPathTemp, char* rawPathTemp, char* depthPathTemp, char* matrixPathTemp) {
	WCHAR imgPath[fileLength];
	swprintf(imgPath, fileLength, imgPathTemp, id, 1);
	char rawPath[fileLength];
	sprintf(rawPath, rawPathTemp, id);
	char depthPath[fileLength];
	sprintf(depthPath, depthPathTemp, id);
	char matrixPath[fileLength];
	sprintf(matrixPath, matrixPathTemp);

	int t = 0;
	while (!fileExist(rawPath)) {
		//captute image with depth and stencil
		catchCurveAndScreen(imgPath, rawPath, depthPath, matrixPath, NULL, false);  // catchCurveAndScreen
		//catchCurveAndScreen(imgPath, rawPath, depthPath, matrixPath, false);
		WAIT(1); t++;
	}
	log_to_file("catch depth, stencil and screen, time = " + std::to_string(t));
}
...
void createOneScene(int hashlen, float sx, float dx, float sy, float dy, std::string weather, int pedNum = 520) {
	...
	// step 3: create crowd image information file
	char foldName[fileLength], pedInfoFile[fileLength],rawPathTemp[fileLength], depthPathTemp[fileLength], matrixPathTemp[fileLength], envDepthTemp[fileLength];
	WCHAR imgPathTemp[fileLength];
	createImgFold(foldName, pedInfoFile, rawPathTemp, imgPathTemp, depthPathTemp, matrixPathTemp, envDepthTemp);  // create image file and so on

	// step 4: create scene and cap screen
	const int maxPedOneTime = 180;  // 250
	...
	while (ms < pedNum) {
		...
		{
			Vector3 boneLoc = PED::GET_PED_BONE_COORDS(thisPed.pedActor, headCode, 0, 0, 0);  // head bone loc
			float hx, hy;
			BOOL getCoord = GRAPHICS::_WORLD3D_TO_SCREEN2D(boneLoc.x, boneLoc.y, boneLoc.z, &hx, &hy);  // 3D to 2D loc
			shx = std::to_string(hx);  // head x
			shy = std::to_string(hy);  // head y
		};
		for (int i = ms; i < m; i++) {
		...
		/* capture stencial raw file */
		screenStencilCap(ms, imgPathTemp, rawPathTemp, depthPathTemp, matrixPathTemp);
		WAIT(1000);
		...
		WAIT(1000);
		/* capture background */
		screenCap(ms, imgPathTemp, rawPathTemp, depthPathTemp, envDepthTemp);
...
void createScene(int nowHave) {
	...
	// read info for this place
	float mix, mxx, miy, mxy;
	readAreaBorder(mix, mxx, miy, mxy);  // areaInfo.log
	float cx = (mix + mxx) / 2, cy = (miy + mxy) / 2, camX, camY, camZ;
	getCameraLoc(camX, camY, camZ);  // eyeInfo.log
	...
	// put camera and save camera parameters
	showCamera(pedestrian::camX, pedestrian::camY);  // todo

	log_to_file("start time: " + std::to_string(time(NULL)));
	auto runSceneCreator = [&](int avg2) {
		int m = readImgNum();  // 5->20
		log_to_file("m = " + std::to_string(m));
		// for each image
		for (int i = nowHave; i < m; i++) {
			log_to_file("i = " + std::to_string(i));
			std::string weather = setTimeAndWeather();                // environment
			// log_to_file("avg2 = " + std::to_string(avg2));
			//int pedNum = avg2 * 0.25 + rand() % int(avg2 * 0.75) + 1;  // ped number
			int pedNum = avg2 * 0.65 + rand() % int(avg2 * 0.4) + 1;
			/* create one scene */
			createOneScene(281, mix, mxx - mix, miy, mxy - miy, weather, pedNum);
		}
	};  // <-
	runSceneCreator(readLevelFile());  // levelInfo.log -> runSceneCreator
