...
bool InitPartNo()
{
	...
	for (auto & p : std::experimental::filesystem::v1::directory_iterator(root_dir)) {
		...
		if (st != -1) {
			...
			std::string imgFold;
			int thisImgNum = 0;
			log_to_file("subFold = " + subFold);  // subFold = .\data\part_9_1

			/* iterate all file in subFolder */
...
int defaultFold()
{
	int imgNum = readImgNum(), sceneNum = parts.size();  // sceneNum - number of part folder
	log_to_file("imgNum and sceneNum = " + std::to_string(imgNum) + " " + std::to_string(sceneNum));  // 5, 45
	
	log_to_file("workId = " + std::to_string(workId));
	workId += 1;
	if (workId >= sceneNum)
		return -1;
	/*
	...
	*/
	...
	/* change folder now */
	std::string foldName = "part_" + std::to_string(parts[workId]) + "_" + std::to_string(subParts[workId]);  // select
	strcpy(nowFold, "data\\");
	strcat(nowFold, foldName.c_str());  // select a part(scene)
	log_to_file("deault fold is " + std::string(foldName));  //  deault fold is part_1_0
...
void readAreaBorder(float &mix, float &mxx, float &miy, float &mxy)
{
	char areaInfoFile[fileLength] = "areaInfo.log";
	foldCat(areaInfoFile);
	log_to_file(areaInfoFile);  // data\part_1_1\areaInfo.log
